...
export type StoredState = { transactions: StoredTransaction[]; stockHoldings: StoredStockHolding[]; updatedAt: string };
...
export async function writeState(next: Pick<StoredState, "transactions" | "stockHoldings">) {
  const state: StoredState = { transactions: next.transactions, stockHoldings: next.stockHoldings, updatedAt: new Date().toISOString() };
  const sql = await ensureTable();
  if (sql) {
